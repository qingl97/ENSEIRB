! Poisson 方程求解改进一: 使用非阻塞通信。作者: 莫则尧
      INCLUDE  'mpif.h'
...
! 程序可执行语句开始
	 ...
	 CALL MPI_Finalize(IERR)
         STOP
      ENDIF
! 按自然序 (先沿 X 方向, 后沿 Y 方向) 确定各进程自身及其 4 个相邻进程的进程号\label{poisson1:1}
      ...
      MEPY=MYRANK/NPX
      MEPX=MYRANK-MEPY*NPX	!\label{poisson1:2}
...
! 非阻塞地交换定义在辅助网格结点上的近似解
     ...
     &               MPI_COMM_WORLD, REQ(8),IERR)           ! 接收下边界
      ...
      DO J=JST,JEND	!\label{poisson1:3}
      DO I=IST,IEND
         U0(I,J)=F(I,J)+DX*(U(I,J-1)+U(I,J+1))+DY*(U(I-1,J)+U(I+1,J))
      ENDDO
      ENDDO		!\label{poisson1:4}
! 计算与精确解间的误差
      ERR=0.0
      DO J=JST,JEND
      DO I=IST,IEND
