...
void usage(char *msg){
    ...
    fprintf(stderr, "Usage: ./shear {h,v} angle source_image dest_image\n");
}

/*Map the current pixel at destination image to the pixel at the source image*/
void getLocation(char dir, int x, int y, double *x_d, double *y_d){
...
void bilinear_interpolation(pnm image, double x, double y, unsigned char *pixel){
    ...
    if(s_x==0 || s_y==0 || s_y+1>=s_height || s_x+1>=s_width)
        for(int i=0; i<3; i++){
            pixel[i] = *val_0++;
        }
    else {
...
void shear(char* path_ims, char *path_imd, char dir, double angle){
    //printf("begin shearing ...\n");
...
int main(int argc, char* argv[]){
    ...
    dir = *(argv[1]);
    angle = atof(argv[2]);
    char *path_ims = argv[3];
    char *path_imd = argv[4];
    shear(path_ims, path_imd, dir, angle);
